...
export default function AdminDashboard() {
  ...
  const handleEditProduct = async (productData) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://my-ecommerce-production-11b0.up.railway.app'}/api/products/${editingProduct._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const confirmDeleteProduct = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://my-ecommerce-production-11b0.up.railway.app'}/api/products/${productToDelete._id}`, {
        method: 'DELETE',
      });
